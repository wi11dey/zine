...
commutativeTests = describe "Commutative operations" $ do
    it "Add" $ do
        let input = "{1 + 2}"
        runParser pText input
            `shouldBe` Just
                ( Text
                    [ TMath
                        (MInline (ECommutative (EAtom (AInteger "1")) " + " (EAtom (AInteger "2"))))
                    ]
                )
            ...
            `shouldBe` Just
                ( Text
                    [ TMath
                        ( MInline
                            ( ECommutative
                                (ECommutative (EAtom (AInteger "1")) " + " (EAtom (AInteger "2")))
                                " + "
                                (EAtom (AInteger "3"))
                            )
                        )
...
quantumTests = describe "Quantum notation" $ do
    it "Bra" $ do
        let input = "{<psi|}"
        runParser pText input
            `shouldBe` Just (Text [TMath (MInline (EBra (EAtom (AIdent "psi"))))])

    it "Ket" $ do
        let input = "{|psi>}"
        runParser pText input
            `shouldBe` Just (Text [TMath (MInline (EKet (EAtom (AIdent "psi"))))])
    ...
    it "Projection" $ do
        ...
        runParser pText input
            `shouldBe` Just
                ( Text
                    [TMath (MInline (EProjection (EAtom (AIdent "psi")) (EAtom (AIdent "psi"))))]
                )
    ...
    it "Open" $ do
        ...
        runParser pText input
            `shouldBe` Just
                ( Text
                    [ TMath
                        (MInline (EInterval '(' (EAtom (AInteger "1")) (EAtom (AInteger "2")) ')'))
                    ]
                )
    ...
    it "Closed" $ do
        ...
        runParser pText input
            `shouldBe` Just
                ( Text
                    [ TMath
                        (MInline (EInterval '[' (EAtom (AInteger "1")) (EAtom (AInteger "1")) ']'))
                    ]
                )
...
negationTests = describe "Negation" $ do
    it "Negation" $ do
        let input = "{-x}"
        runParser pText input
            `shouldBe` Just (Text [TMath (MInline (ENegation (EAtom (AIdent "x"))))])
    ...
    it "Double negation with parens" $ do
        ...
        runParser pText input
            `shouldBe` Just
                (Text [TMath (MInline (ENegation (EParens (ENegation (EAtom (AIdent "x"))))))])
            ...
            `shouldBe` Just
                ( Text
                    [ TMath
                        ( MInline
                            ( EAtom
                                (AScript (Superscript (BAtom (AInteger "2")) (BNegation (EAtom (AIdent "x")))))
                            )
                        )
                    ]
                )
        ...
        runParser pText input
            `shouldBe` Just
                ( Text
                    [ TMath
                        ( MInline
                            (EAtom (AScript (Subscript (BAtom (AInteger "2")) (BAtom (AInteger "2")))))
                        )
                    ]
                )
                        ...
                        ( MInline
                            ( EAtom
                                ( AScript
                                    ( Subscript
                                        (BAtom (AInteger "2"))
                                        (BAtom (AScript (Subscript (BAtom (AInteger "2")) (BAtom (AInteger "2")))))
                                    )
                                )
                            )
                        )
...
setTests = describe "Set notation" $ do
    it "Set" $ do
        ...
        runParser pText input
            `shouldBe` Just
                (Text [TMath (MInline (ESetBuilder (EAtom (AIdent "x")) (EAtom (AIdent "x"))))])
        ...
        runParser pText input
            `shouldBe` Just
                ( Text
                    [ TMath
                        ( MInline
                            (ESet [EElem (EAtom (AInteger "1")), EElem (EAtom (AInteger "2")), EEllipsis])
                        )
                    ]
                )
...
spanTests = describe "Spans" $ do
    it "Span" $ do
        ...
        runParser pTextEof input
            `shouldBe` Just (Text [TSpan "def" (Text [TWord "convergent"])])
    ...
    it "Embedded span" $ do
        ...
        runParser pTextEof input
            `shouldBe` Just
                ( Text
                    [ TWord "A",
                      TSpan "def" (Text [TWord "convergent", TWord "sequence"]),
                      TWord "is",
                      TWord "notated",
                      TWord "as",
                      TMath (MInline (EAtom (AIdent "asdf")))
                    ]
                )
    ...
    it "Keywords on their own" $ do
        let input = "A def b"
        runParser pTextEof input
            `shouldBe` Just (Text [TWord "A", TWord "def", TWord "b"])
            ...
            `shouldBe` Just
                ( Text
                    [ TMath
                        ( MInline
                            ( ENoncomm
                                (ENoncomm (EAtom (AIdent "sin")) (EAtom (AIdent "k")))
                                (EAtom (AIdent "a"))
                            )
                        )
                    ]
                ...
                ( Text
                    [ TMath
                        ( MInline
                            ( ENoncomm
                                (EAtom (AIdent "sin"))
                                (ENoncomm (EAtom (AIdent "k")) (EAtom (AIdent "a")))
                            )
                        )
                    ]
                )
                    ...
                    [ TMath
                        ( MInline
                            ( ENoncomm
                                ...
                                ( ENoncomm
                                    (EAtom (AIdent "k"))
                                    (ENoncomm (EAtom (AIdent "a")) (EAtom (AIdent "b")))
                                )
                            )
                        )
                    ]
...
parensTests = describe "Parentheses" $ do
    it "One" $ do
        let input = "{(1)}"
        runParser pText input
            `shouldBe` Just (Text [TMath (MInline (EParens (EAtom (AInteger "1"))))])

    it "Two" $ do
        let input = "{((1))}"
        runParser pText input
            `shouldBe` Just (Text [TMath (MInline (EParens (EParens (EAtom (AInteger "1")))))])

    it "Three" $ do
        let input = "{(((1)))}"
        runParser pText input
            `shouldBe` Just
                (Text [TMath (MInline (EParens (EParens (EParens (EAtom (AInteger "1"))))))])
            ...
            `shouldBe` Just
                ( Text
                    [ TMath
                        ( MInline
                            ( EParens
                                (EParens (EInterval '(' (EAtom (AInteger "1")) (EAtom (AInteger "2")) ')'))
                            )
                        )
                    ]
                )
                ...
                ( Text
                    [ TMath
                        ( MInline
                            ( EParens
                                ( EParens
                                    (ENoncomm (EParens (EAtom (AInteger "1"))) (EParens (EAtom (AInteger "1"))))
                                )
                            )
                        )
                    ]
                        ...
                        ( MInline
                            ( ECommutative
                                ( ECommutative
                                    ( ENoncomm
                                        (EAtom (AIdent "a"))
                                        (EAtom (AScript (Superscript (BAtom (AIdent "x")) (BAtom (AInteger "2")))))
                                    )
                                    " + "
...
calculusTests = describe "Calculus" $ do
    it "Integrals" $ do
        let input = "{int}"
        runParser pText input
            `shouldBe` Just (Text [TMath (MInline (EAtom (AIdent "int")))])
